...
bitboard get_black_pawn_moves(bitboard pieces, bitboard opp_pieces, bitboard en_passant, Move move_list[], int src,
                              int *moveIndex) {
    //Sets up attacks.
...
bitboard get_white_pawn_moves(bitboard pieces, bitboard opp_pieces, bitboard en_passant, Move move_list[], int src,
                              int *moveIndex) {
    //Sets up attacks.
    bitboard mask = 1ULL << src;
    bitboard attacks = 0b101;
...
int get_move_list(Board *board, Move *move_list) {
    ...
    while (pieces) {
        POP_BSF(src, pieces);
        bitboard attacks = board->white ? get_white_pawn_moves(all_pieces, op_pieces, board->en_passant, move_list, src,
                                                               &list_index) : get_black_pawn_moves(all_pieces,
                                                                                                   op_pieces,
                                                                                                   board->en_passant,
                                                                                                   move_list, src,
                                                                                                   &list_index);
